...
struct OptionalBitmapAndOp : public BitmapOperation {
  Result<std::shared_ptr<Buffer>> Call(MemoryPool* pool, const uint8_t* left,
    ...
    ARROW_ASSIGN_OR_RAISE(auto right_buffer,
                          CopyBitmap(pool, right, 0, length + right_offset));
    ARROW_ASSIGN_OR_RAISE(auto left_buffer,
                          CopyBitmap(pool, left, 0, length + left_offset));
  ...
  Status Call(const uint8_t* left, int64_t left_offset, const uint8_t* right,
    ...
    ARROW_ASSIGN_OR_RAISE(auto right_buffer, CopyBitmap(default_memory_pool(), right, 0,
                                                        length + right_offset));
    ARROW_ASSIGN_OR_RAISE(auto left_buffer, CopyBitmap(default_memory_pool(), left, 0,
                                                       length + left_offset));
    ARROW_ASSIGN_OR_RAISE(
        auto out, AllocateEmptyBitmap(length + out_offset, default_memory_pool()));
    OptionalBitmapAnd(left_buffer, left_offset, right_buffer, right_offset, length,
                      out_offset, out);
    out_buffer = out->mutable_data();
    return Status::OK();
  }
};
